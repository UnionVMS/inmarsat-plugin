...
public class InmarsatHeader {
    ...
    private static boolean isValidHeaderLength(byte[] headerToValidate) {
        if (headerToValidate == null) return false;
        HeaderType type = HeaderType.fromInt(headerToValidate[HeaderStruct.POS_TYPE]);
        if (type == null) return false;
        boolean b1 = (HeaderStruct.POS_REF_NO_END + 1) < headerToValidate.length; // MIN LENGTH
        boolean b2 = headerToValidate[HeaderStruct.POS_HEADER_LENGTH] == headerToValidate.length;
        boolean b3 = type.getHeaderLength() == headerToValidate.length;
        return b1 && b2 && b3;
    }
    ...
    public int getLesId() {
        Integer position = getPositionSatIdAndLesId();
    ...
    private Integer getPositionSatIdAndLesId(){
